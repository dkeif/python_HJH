...
def draw_rainbow(t, rainbow_size, pen_size, x, y):
    ...
    for i in range(497):
        t.setheading(90)
        t.penup()
        t.setpos(x+(rainbow_size - (i * pen_size)), y)
        t.pencolor(rainbow_color[i%7])
        t.pendown()
...
'''
numbers = (1, 4, 28, 5, 302, 40)
numbers = list(numbers)                         # 튜플을 list로 변환하기
print(numbers, type(numbers))              #list
numbers = tuple(numbers)                         # 다시 list를 tuple로 변환해주기
print(numbers, type(numbers))              #tuple
'''
...
print("numbers:", numbers, type(numbers))   # 결과확인((12, 13), tuple)

# 언패킹
c, d = numbers     # numbers에 포함된 숫자를 c, d로 언패킹해주기
print("c: ", c, type(c))                    # 결과확인(12, int)
